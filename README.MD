...
#### -> the last nginx is the image in dockerhub that it pulls from which it builds a container..remember each container is built from a image that means you can run several container from a image. So, there can be multiple instances (container) of a image.
